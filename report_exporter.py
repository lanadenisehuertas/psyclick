...
def _group_name(gid):
    return {1: "Time Pressure & Workload", 2: "Interpersonal Friction",
            3: "Performance Pressure", 4: "Self-Evaluation & Identity"}.get(gid, f"Group {gid}")
...
def generate_html_report(data):
    ...
    if ts_raw:
        try:
            ...
            utc_dt = datetime.fromisoformat(ts_raw.replace(' ', 'T'))
    ...
    domain_svg = ""
    if dom:
        colors = {1:"#3B82F6",2:"#0ABFBC",3:"#F59E0B",4:"#EF4444"}
        names  = {1:"Workload",2:"Interpersonal",3:"Performance",4:"Self-Eval"}
...
def _build_recs_for_html(ae, phq, gad, domain_t2, level_t2):
    ...
    if domain_t2:
        ...
        names = {1:"Time Pressure & Workload",2:"Interpersonal Friction",
                 3:"Performance Pressure",4:"Self-Evaluation & Identity"}
        tips  = {1:"Explore workload management, time anxiety, and boundary-setting.",
                 ...
                 3:"Explore performance anxiety, evaluation fear, and body-based stress responses.",
...
def generate_summary_html(rows):
    ...
    for pid, ts, flag, phq, gad, psi, pai, label in rows:
        ...
        if ts:
            try:
                from datetime import timezone
                utc_dt = datetime.fromisoformat(ts.replace(' ', 'T'))
                if utc_dt.tzinfo is None:
                    utc_dt = utc_dt.replace(tzinfo=timezone.utc)
                ts_local = utc_dt.astimezone().strftime("%Y-%m-%d %H:%M")
            except Exception:
                pass
        ...
        <tr>
          ...
          <td style="color:#64748B;font-size:11px">{ts_local}</td>
