...
const GLOSSARY = {
  ...
  PSI: {
    title: "Psychomotor Slowing Index",
    what: "Measures motor slowing — slow typing, long pauses, deliberate cursor movement — relative to this patient's own calibration baseline.",
    flag: "Ipsative thresholds (vs. own baseline): PSI > 0.5 mild · > 2.0 moderate · > 4.0 severe. When Population Comparison data is available, prefer the normative z-score: PSI is naturally elevated even in healthy people across a full 11-prompt session, so z > +1.5 is the meaningful clinical signal. Correlates with MADRS items 6–7.",
    how: "Z-scored composite of keystroke flight time + pause frequency vs. patient's calibration baseline.",
  },
  PAI: {
    title: "Psychomotor Agitation Index",
    what: "Measures motor agitation — erratic cursor, irregular keystrokes, path deviation — relative to this patient's own calibration baseline.",
    flag: "Ipsative thresholds (vs. own baseline): PAI > 0.5 mild · > 2.0 moderate · > 4.0 severe. When Population Comparison data is available, prefer the normative z-score: PAI accumulates across 11 prompts even in healthy individuals, so z > +1.5 is the meaningful clinical signal. Correlates with HAM-A items 1–4.",
    how: "Z-scored composite of cursor jerk, curvature, and velocity variance vs. mouse calibration baseline.",
  },
  DomainT2: {
    title: "Domain T² Scores",
    what: "Which emotional theme triggered the most body-language change. 4 domains: Time/Workload, Interpersonal, Performance, Self-Evaluation.",
    ...
    how: "Mean T² of prompts within each domain (Levels A, B, C where applicable).",
  },
  LevelABC: {
...
const DOMAIN_NAMES  = { 1: 'Time/Workload', 2: 'Interpersonal', 3: 'Performance', 4: 'Self-Eval' }
const DOMAIN_COLORS = { 1: '#5BA4CF', 2: '#0ABFBC', 3: '#F59E0B', 4: '#F27C7C' }
const DOMAIN_BG     = { 1: '#EFF6FF', 2: '#E0FAFA', 3: '#FFFBEB', 4: '#FFF0F0' }
...
function clinicalRecs(flag, label = '', psi = 0, pai = 0, phq = 0, gad = 0, domainT2 = {}, levelT2 = {}) {
  ...
  if (domEntries.length > 0) {
    ...
    const names = { 1: 'Time Pressure & Workload', 2: 'Interpersonal Friction', 3: 'Performance Pressure', 4: 'Self-Evaluation & Identity' }
    const tips  = {
      ...
      3: 'Explore performance anxiety, evaluation-related threat, and body-based stress responses. Assess for social anxiety disorder and impostor phenomenon. Consider LSAS if social anxiety is suspected.',
    ...
    <div className="card p-6">
      ...
      <div className="overflow-hidden rounded-lg border border-border/50">
        ...
        >
          ...
          <defs>
            <clipPath id="hm-clip">
              <rect x={ML} y={-100} width={plotW} height={plotH + 200} />
            </clipPath>
          </defs>
...
function NormativeComparison({ metrics }) {
  if (!metrics || Object.keys(metrics).length === 0) return null

  const SHOW = ['t2_score', 'psi', 'pai', 'phq_score', 'gad_score']
  const entries = SHOW.map(k => metrics[k]).filter(Boolean)
  ...
  return (
    <div className="card p-6">
      <div className="flex items-center gap-2 mb-1">
        ...
        <span className="ml-auto text-xs text-tsub">vs. normative baseline (n={entries[0]?.count ?? '?'})</span>
      </div>
      <p className="text-xs text-tsub mb-4 leading-relaxed">
        How this client's scores compare to the normative population (healthy baseline group).
        Z-scores show standard deviations from the population mean; percentile shows where the client ranks.
      </p>
      ...
      <p className="text-xs text-tsub mt-3 border-t border-border/50 pt-3">
        Population comparison is separate from the ipsative (within-session) comparison shown in the metrics above.
        Ipsative analysis compares the client's current session to their own calibration baseline;
        population comparison shows where they fall relative to the normative reference group.
      </p>
    </div>
  )
}
  ...
  useEffect(() => {
    if (sessionId) {
      api.sessionDetail(sessionId).then(d => {
        if (!d || d.success === false) { setErr(d.detail || d.error || 'Failed to load session'); return }
        setData(d)
      })
      api.normativeCompare(sessionId).then(d => {
        if (d.available) setNormComp(d.metrics)
      })
    } else if (ctxReport) {
      setData(ctxReport)
      // For fresh sessions, try compare by looking at latest session
      // (sessionId not known yet — normative compare will be available when revisiting)
    }
  }, [sessionId, ctxReport])
    ...
    <div className="h-screen flex bg-bg" style={{ position: 'relative' }}>
      ...
      <main className="flex-1 overflow-y-auto px-10 py-8 animate-fade-in" style={{ position: 'relative', zIndex: 1 }}>
        ...
        <div className="w-full space-y-4">
          ...
          <NormativeComparison metrics={normComp} />
