...
export default function EmotionalTask() {
  ...
  async function handleFinish() {
    ...
    if (res.success && res.report) {
      ...
      navigate(user?.role === 'normer' ? '/normative/complete' : '/report')
    } else {
  ...
  return (
    ...
    <AnimatedBackground />
      <button
        ...
        className="fixed left-6 top-6 z-40 inline-flex items-center gap-2 rounded-full border border-border bg-white/90 px-5 py-3 text-base font-bold text-tmain shadow-card backdrop-blur transition-colors hover:border-accent/50 hover:text-accent"
      >
        <LogOut size={20} />
        Back to Login
      </button>
      ...
      <div className="flex min-h-screen flex-col items-center px-6 py-10 animate-fade-in">
        ...
        <div className="w-full max-w-5xl">
          ...
          <div className="mb-6 text-center">
            <h1 className="mb-2 text-4xl font-bold text-tmain">Emotional Response Task</h1>
            <p className="text-lg text-tsub">Question {qi + 1} of {total} - respond in your own words</p>
          </div>

          <div className="mb-5 h-3 overflow-hidden rounded-full bg-border">
          ...
          <div className="card mb-5 p-7">
            ...
            <textarea
              ...
              autoComplete="off"
            />
            <p className={`mt-3 text-right text-sm font-semibold ${response.length >= 480 ? 'text-coral' : 'text-tsub'}`}>
              {response.length}/500
            </p>
          </div>
