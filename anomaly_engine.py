...
def _obs_mask(feature_dict):
    ...
    return np.array([
        ...
        has_mouse,  # 6  jerk
        has_kbd,    # 7  pause_frequency (only compare keyboard to keyboard)
    ])
...
def _t2_threshold(n, p=N_FEATURES, alpha=0.05):
    ...
    """
    if n <= p + 5:  # Use chi2 for small n to avoid F-distribution instability when n-p is small
...
def fuzzy_classify(t2, threshold, psi, pai, norm_stats=None):
    """
    ...
    # CALIBRATED FUZZY PARAMETERS — Data-driven from 110-session normative baseline
    # Ranges set to p99 of healthy population distributions
    t2_n     = _normalise(t2_ratio,       0.0, 7.00)    # p99 of t2/threshold ratio = 6.9975

    if norm_stats and 'psi' in norm_stats and 'pai' in norm_stats:
        psi_mean = norm_stats['psi']['mean']
        psi_sd   = norm_stats['psi']['sd']
        pai_mean = norm_stats['pai']['mean']
        pai_sd   = norm_stats['pai']['sd']

        # Use normative mean and 2*SD for normalization
        psi_n    = _normalise(psi, psi_mean, psi_mean + 2 * psi_sd)
        pai_n    = _normalise(pai, pai_mean, pai_mean + 2 * pai_sd)
    else:
        # Fallback to p99-calibrated values if normative baseline not available
        # From actual 110-session normative population
        psi_n    = _normalise(max(psi, 0.0),  0.0, 39.10)   # p99 of PSI = 39.1050
        pai_n    = _normalise(max(pai, 0.0),  0.0, 107.83)  # p99 of PAI = 107.8280
    ...
    # Thesis spec (§3.5.2, Decision Rule Mapping):
    #   RED   — T² > 1.5× threshold AND severe fuzzy rules dominate
    #   AMBER — T² between threshold and 1.5× threshold, or borderline rules dominate
    #   GREEN — T² within threshold (Normal dominant)
    if t2_ratio <= 1.0:
        # T² within session baseline — always GREEN regardless of PSI/PAI
        flag = "GREEN"
    elif t2_ratio > 1.5 and severe_strength > 0 and severe_strength >= borderline_strength:
        # T² significantly exceeds threshold AND severe psychomotor signal — RED
        flag = "RED"
    elif borderline_strength > 0 or severe_strength > 0:
        # T² above threshold but < 1.5×, or severe rules fire but threshold not reached — AMBER
        flag = "AMBER"
    else:
        # T² is high but no specific psychomotor profile matches -- flag as AMBER (unusual) instead of defaulting to GREEN
        flag = "AMBER"
...
class AnomalyEngine:
    ...
    def analyse(self, feature_dict, norm_stats=None):
        """
        ...
        fuzzy             = fuzzy_classify(t2, thresh, psi, pai, norm_stats=norm_stats)
