        ...
        <div className="task-panel">
          ...
          <div className="card p-8">
            ...
            <div className="mt-3 h-3 overflow-hidden rounded-full bg-border">
              <div
                className={`h-full rounded-full transition-all duration-300 ${canContinue ? 'bg-success' : 'bg-accent'}`}
                style={{ width: `${pct}%` }}
              />
            </div>
            {!canContinue && typed.length > 0 && (
              <p className="mt-2 text-xs text-tsub text-center">
                Type at least {MIN_PCT}% of the passage to continue ({MIN_PCT - pct}% remaining)
              </p>
            )}
          </div>
          ...
          <button
            ...
            className="btn-primary h-14 w-full text-lg disabled:opacity-40 disabled:cursor-not-allowed"
          >
            {canContinue ? 'Continue →' : `Type ${MIN_PCT - pct}% more to continue`}
          </button>
        </div>
      </div>
    </div>
