...
export default function Intake() {
  ...
  async function handleSubmit() {
    ...
    // Accept C-001 through C-999 (or any 3-digit suffix as extended range)
    if (pidRaw && !/^C-\d{3}$/.test(pidRaw)) {
      setErr('Client ID must follow the format C-### (e.g. C-001, C-042).')
      return
    }
    const pid = pidRaw || name.trim() || 'C-UNKNOWN'
    const res = await api.intakeStart(pid, user?.id, consent, '1.0')
