@echo off
echo ============================================================
echo  PsyClick Standard Edition -- Full Production Build
echo  Includes normative tester portal and clinician dashboard.
...
echo.
echo [4/4] Creating Windows installer (PsyClick)...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  "$names=@('PsyClick','psyclick_api','electron','app-builder'); foreach($n in $names){Get-Process -Name $n -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue}; Start-Sleep -Seconds 2; $p='dist-electron\win-unpacked'; if(Test-Path $p){for($i=1;$i -le 5;$i++){try{Remove-Item -LiteralPath $p -Recurse -Force -ErrorAction Stop; break}catch{Write-Host ('Cleanup retry '+$i+': '+$_.Exception.Message); Start-Sleep -Seconds 3}}}"
...
echo.
echo ============================================================
echo  BUILD COMPLETE -- PsyClick Standard Edition
echo ============================================================
echo.
echo  Installer : frontend\dist-electron\PsyClick Setup.exe
echo  Portable  : frontend\dist-electron\PsyClick*.exe
echo.
...
echo  - Includes both tester and clinician interfaces
echo.
pause
